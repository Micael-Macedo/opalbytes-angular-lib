...
export abstract class CaoWebSocketService<TRequest, TResponse> {
  ...
  protected config!: IWebSocketConfig;
   ...
   */
  connect(configOverride?: Partial<IWebSocketConfig>): Observable<boolean> {
    ...
    this.state$.next(WebSocketState.Connecting);

    const connection = new WebSocketConnection(this.config);
  ...
  protected scheduleReconnect(): void {
    ...
    this.state$.next(WebSocketState.Reconnecting);
   ...
   */
  protected abstract getDefaultConfig(): IWebSocketConfig;
   ...
   */
  get messages(): Observable<IWebSocketResponse<TResponse>> {
